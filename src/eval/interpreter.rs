...
impl Interp {
        ...
        match node {
            DefNode::Variable(var) => {
                if let Ok(evaled_var) = self.eval_node(*var.value) {
                    ...
                    let bind = self.env.create_binding(var.name, binding?);
                    if let Err(s) = bind {
                        Err(s.clone())
            ...
            DefNode::Function(fun) => {
                ...
                let bind = self.env.create_binding(fun.name, binding?);
                if let Err(s) = bind {
                    Err(s.clone())
                } else { Ok(AstNode::new_bool_lit(true)) }
            }
    ...
    fn eval_func_call_expr(& self, call: FuncCallData) -> Result<AstNode, String> {
        let binding = self.env.get_binding(&call.name);
        if let Some(Binding { value: LitNode::Lambda(lambda), .. }) = binding {
            let env: Rc<Environment>;
            if lambda.def.parameters.is_some() && call.arguments.is_some() {
                ...
                env = Rc::clone(&lambda.env); // Clone the environment directly            } else {
                return Err("Parameter-argument mismatch".to_string());
            }
            let prev_env = std::mem::replace(&mut self.env, env); // swap this new environment with the current active one
            let result = self.eval_node(lambda.def.body);
            self.env = prev_env; // restore the old active one 
            result
        } else {
            // Handle the error case where the binding is not found or not a lambda.
            Err("Binding is not a lambda".to_string())
        }
    }


    fn map_param_to_env(&mut self, mut params: Vec<Param>, mut args: Vec<FuncArg>, mut env: Rc<Environment>) -> Result<Rc<Environment>, String> {
        ...
        if params.len() == args.len() {
            for i in 0..args.len() {
                ...
                env.create_binding(params[i].name.clone(), binding)?;
            }
            Ok(env)
        } else {
            panic!();
    ...
    fn eval_lit_call_expr(&self, name: String) -> Result<AstNode, String> {
        if let Some(lit) = self.env.get_literal(&name) {
