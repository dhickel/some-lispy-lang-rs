use crate::parse::{Definition, Expression, lexer, Literal, Syntactic, TokenData};
use crate::parse::TokenType::{DefinitionToken, ExpressionToken, LiteralToken, SyntacticToken};
...
pub fn test_keyword_identity_lexing(){
    ...
    let result = lexer::process(input).expect("Lexing Failed");
    assert!(matches!(result.get(1).unwrap().token_type, DefinitionToken(Definition::Define)));
    assert!(matches!(result.get(2).unwrap().token_type, LiteralToken(Literal::Identifier)));
    assert!(matches!(result.get(6).unwrap().token_type, ExpressionToken(Expression::ForI)));
    assert!(matches!(result.get(12).unwrap().token_type, LiteralToken(Literal::Identifier)));
    assert!(matches!(result.get(15).unwrap().token_type, LiteralToken(Literal::Identifier)));
    assert!(matches!(result.get(18).unwrap().token_type, LiteralToken(Literal::Identifier)));
...
pub fn test_type_lexing(){
    ...
    let result = lexer::process(input).expect("Lexing Failed");
    assert!(matches!(result.get(3).unwrap().token_type, SyntacticToken(Syntactic::Type)));
