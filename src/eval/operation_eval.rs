...
pub fn add<'a>(data: (bool, Vec<LitNode>)) -> Cow<'a ,AstNode> {
    if data.0 {
...
pub fn subtract<'a>(data: (bool, Vec<LitNode>)) -> Cow<'a ,AstNode> {
    if data.0 {
        let mut result: f64 = 0.0;
...
pub fn multiply<'a>(data: (bool, Vec<LitNode>)) -> Cow<'a ,AstNode> {
    if data.0 {
        let mut result: f64 = 0.0;
        for i in 0..data.1.len() {
            if i == 0 {
...
pub fn divide<'a>(data: (bool, Vec<LitNode>)) -> Cow<'a ,AstNode> {
    if data.0 {
        let mut result: f64 = 0.0;
        for i in 0..data.1.len() {
            if i == 0 {
...
pub fn modulo<'a>(data: (bool, Vec<LitNode>)) -> Cow<'a ,AstNode> {
    if data.0 {
        let mut result: f64 = 0.0;
        for i in 0..data.1.len() {
            if i == 0 {
...
pub fn exponentiate<'a>(data: (bool, Vec<LitNode>)) -> Cow<'a ,AstNode> {
    let mut result: f64 = 0.0;
    for i in 0..data.1.len() {
        if i == 0 {
            result = data.1[i].value().as_float();
...
pub fn increment<'a>(data: (bool, Vec<LitNode>)) -> Cow<'a ,AstNode> {
    if data.0 {
        ...
        if result.0.len() == 1 {
            Cow::Owned( LiteralNode(result.0.remove(0)))
        } else {  Cow::Owned(LiteralNode(LitNode::Vector(result))) }
    } else {
        let mut result: VectorValue = data.1.into_iter()
...
pub fn decrement<'a>(data: (bool, Vec<LitNode>)) -> Cow<'a ,AstNode> {
    if data.0 {
        let mut result: VectorValue = data.1.into_iter()
            .map(|item| LitNode::Float(FloatValue(item.value().as_float() - 1.0)))
            .collect();
...
pub fn or<'a>(data: (bool, Vec<LitNode>)) -> Cow<'a ,AstNode> {
    for op in data.1 {
...
pub fn and<'a>(data: (bool, Vec<LitNode>)) -> Cow<'a ,AstNode> {
    for op in data.1 {
...
pub fn xor<'a>(data: (bool, Vec<LitNode>)) -> Cow<'a ,AstNode> {
    let mut truths = 0;
    for op in data.1 {
...
pub fn nand<'a>(data: (bool, Vec<LitNode>)) -> Cow<'a ,AstNode> {
    for op in data.1 {
        if !op.value().as_bool() { return  Cow::Owned(AstNode::new_bool_lit(true)); }
    }
    Cow::Owned(AstNode::new_bool_lit(false))
...
pub fn nor<'a>(data: (bool, Vec<LitNode>)) -> Cow<'a ,AstNode> {
    for op in data.1 {
        if op.value().as_bool() { return Cow::Owned(AstNode::new_bool_lit(false)); }
    }
    Cow::Owned(AstNode::new_bool_lit(true))
...
pub fn xnor<'a>(data: (bool, Vec<LitNode>)) -> Cow<'a ,AstNode> {
    let mut truths = 0;
    for op in data.1 {
        if op.value().as_bool() { truths += 1 }
    }
...
pub fn negate<'a>(data: (bool, Vec<LitNode>)) -> Cow<'a ,AstNode> {
...
pub fn greater_than<'a>(data: (bool, Vec<LitNode>)) -> Cow<'a ,AstNode> {
    let operands = data.1;
    if data.0 {
...
pub fn greater_than_eq<'a>(data: (bool, Vec<LitNode>)) -> Cow<'a ,AstNode> {
    let operands = data.1;
    if data.0 {
        for i in 1..operands.len() {
            if !(operands[i - 1].value().as_float() >= operands[i].value().as_float()) {
...
pub fn less_than<'a>(data: (bool, Vec<LitNode>)) -> Cow<'a ,AstNode> {
    let operands = data.1;
    if data.0 {
        for i in 1..operands.len() {
            if !(operands[i - 1].value().as_float() < operands[i].value().as_float()) {
...
pub fn less_than_eq<'a>(data: (bool, Vec<LitNode>)) -> Cow<'a ,AstNode> {
    let operands = data.1;
    if data.0 {
        for i in 1..operands.len() {
            if !(operands[i - 1].value().as_float() <= operands[i].value().as_float()) {
...
pub fn value_equality<'a>(data: (bool, Vec<LitNode>)) -> Cow<'a ,AstNode> {
    let operands = data.1;
    for i in 1..operands.len() {
...
pub fn value_non_equality<'a>(data: (bool, Vec<LitNode>)) -> Cow<'a ,AstNode> {
    let operands = data.1;
    for i in 1..operands.len() {
        if operands[i - 1].value().equal_to(&operands[i]) {
            return  Cow::Owned(AstNode::new_bool_lit(false));
...
pub fn ref_equality<'a>(data: (bool, Vec<LitNode>)) -> Cow<'a ,AstNode> {
    let operands = data.1;
    for i in 1..operands.len() {
